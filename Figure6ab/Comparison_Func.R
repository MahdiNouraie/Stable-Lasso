# This script generates comparison of differenrt functional forms of w
...
generate_data <- function(n, p) {
  ...
  rho_values <- c(0.8, 0.85, 0.9, 0.95, 0.99)
...
# Stability Selection with Stable Lasso (w <- 1 - (1/(1 + ranked_features)))
...
# Stability Selection with Stable Lasso (w <- 1 - (1/(1 + log(ranked_features))))
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
...
# Stability Selection with Stable Lasso (w <- 1 - (1/(ranked_features)))
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
...
# Stability Selection with Stable Lasso (w <- 1 - exp(-ranked_features))
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
...
tmp_df <- results[, c("S1", "S2", "S3", "S4")] # Extract stability values
...
tmp_df$Method[tmp_df$Method == "S1"] <- "1 - 1/(1+r)" # Rename methods
tmp_df$Method[tmp_df$Method == "S2"] <- "1 - 1/(1+log(r))" # Rename methods
tmp_df$Method[tmp_df$Method == "S3"] <- "1 - 1/r" # Rename methods
tmp_df$Method[tmp_df$Method == "S4"] <- "1 - exp(-r)" # Rename methods
...
metrics_results1 <- calculate_F1(results, c(6, 62, 63, 64)) # Calculate F1-score for thresholds 0.6
metrics_results2 <- calculate_F1(results, c(7, 72, 73, 74)) # Calculate F1-score for thresholds 0.7
metrics_results3 <- calculate_F1(results, c(8, 82, 83, 84)) # Calculate F1-score for thresholds 0.8
metrics_results4 <- calculate_F1(results, c(9, 92, 93, 94)) # Calculate F1-score for thresholds 0.9
...
metrics_results[, 3] <- rep(c(1, 2, 3, 4), each = 100, times = 4) # Add method column
...
metrics_results$Method[metrics_results$Method == 1] <- "1 - 1/(1+r)" # Rename methods
metrics_results$Method[metrics_results$Method == 2] <- "1 - 1/(1+log(r))" # Rename methods
metrics_results$Method[metrics_results$Method == 3] <- "1 - 1/r" # Rename methods
metrics_results$Method[metrics_results$Method == 4] <- "1 - exp(-r)" # Rename methods
...
write.csv(results, "Comparison_Func_Full.csv", row.names = FALSE) # Save results to CSV file
