# This script generates Figures 3(a) and 3(b) from the paper
...
# Stability Selection with Adaptive Lasso with OLS initials
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
  ...
  # Fit OLS model
  ols_fit <- lm(y ~ x)  # Adds intercept automatically
  
  # Extract coefficients excluding the intercept
  beta_init <- coef(ols_fit)[-1]
...
# Stability Selection with Adaptive Lasso with oracle initials (O)
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
  ...
  # Define oracle
  oracle <- rep(0, p)  # Initialize all coefficients as zero
  active_values <- c(3, 2.5, 2, 1.5, 1)  # Active variable values
  group_size <- p / 5 # Number of variables per group
  active_indices <- seq(group_size, p, by = group_size)  # Last element of each group
  oracle[active_indices] <- active_values  # Assign active variable values
  
  gamma <- 1 # Adaptive LASSO parameter
  w <- 1 / (abs(oracle)^gamma + 1e-6) # Add small value to avoid division by 0
...
tmp_df$Method[tmp_df$Method == "S3"] <- "Adaptive Lasso (OLS)" # Rename methods
tmp_df$Method[tmp_df$Method == "S4"] <- "Adaptive Lasso (O)" # Rename methods
...
metrics_results$Method[metrics_results$Method == 3] <- 'Adaptive Lasso (OLS)' # Rename methods
metrics_results$Method[metrics_results$Method == 4] <- 'Adaptive Lasso (O)' # Rename methods
...
write.csv(results, "Low_Dimension_Full.csv", row.names = FALSE) # Save results to CSV file
