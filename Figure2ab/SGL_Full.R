# This script generates Figures 2(a) and 2(b) from the paper
...
#  [1] SGL_1.3         ggpubfigs_0.0.1 reshape2_1.4.4  ggplot2_3.5.2   glmnet_4.1-8   
#[6] Matrix_1.7-1    MASS_7.3-64    
...
# Stability Selection with Sparse Group Lasso
index <- rep(1:5, each = 200) # Define index for groups
...
tmp_df$Method[tmp_df$Method == "S1"] <- "SGL" # Rename methods
...
metrics_results$Method[metrics_results$Method == 1] <- 'SGL' # Rename methods
...
write.csv(results, "SGL_Full.csv", row.names = FALSE) # Save results to CSV file
