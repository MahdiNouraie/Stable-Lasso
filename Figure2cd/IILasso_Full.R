# This script generates Figures 2(c) and 2(d) from the paper
...
devtools::install_github("https://github.com/tkdmah/iilasso/tree/master", force = TRUE) #installing iilasso package from GitHub
devtools::install_github("JLSteenwyk/ggpubfigs", force = TRUE) # installing ggpubfigs package from GitHub
...
library(iilasso) # Load the iilasso package
...
#  [1] ggpubfigs_0.0.1 iilasso_0.0.3   reshape2_1.4.4  ggplot2_3.5.2  
...
#[45] usethis_3.1.0      miniUI_0.1.1.1     desc_1.4.3         callr_3.7.6       
#[49] pkgconfig_2.0.3    pillar_1.11.0      later_1.4.1        gtable_0.3.6      
#[53] glue_1.8.0         profvis_0.4.0      Rcpp_1.1.0         tibble_3.3.0      
...
# Stability Selection with iilasso
time_taken <- system.time({results <- mclapply(1:25, function(i) {
  ...
  iilasso_cv <- cv_lasso(x, y) # Fit iilasso model with 10-fold CV
  candidate_set <- iilasso_cv$lambda # candidate lambda values
  ...
  for (lambda_idx in seq_along(candidate_set)) {
    ...
    for (j in 1:B) { 
      ...
      iilasso_model <- lasso(x_sub, y_sub, lambda = lambda) # Fit iilasso model
      beta_hat <- as.vector(iilasso_model$beta) # Extract coefficients
...
time_taken <- system.time({results <- mclapply(1:25, function(i) {
  ...
  w <- 1 - (1/ranked_features) # Penalty weights based on ranking
...
tmp_df$Method[tmp_df$Method == "S1"] <- "IILasso" # Rename methods
...
metrics_results[, 3] <- rep(c(1, 2), each = 25, times = 4) # Add method column
...
metrics_results$Method[metrics_results$Method == 1] <- 'IILasso' # Rename methods
...
write.csv(results, "IILasso_Full.csv", row.names = FALSE) # Save results to CSV file
