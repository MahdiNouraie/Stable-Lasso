# This script generates Figures 4(a) and 4(b) from the paper
...
library(ncvreg) # Load the ncvreg package for SCAD
...
#  [1] ggpubfigs_0.0.1 reshape2_1.4.4  ggplot2_3.5.2   ncvreg_3.15.0  
#[5] glmnet_4.1-8    Matrix_1.7-1    MASS_7.3-64    

#loaded via a namespace (and not attached):
#  [1] generics_0.1.3     shape_1.4.6.1      stringi_1.8.7     
...
#[49] pkgconfig_2.0.3    pillar_1.11.0      later_1.4.1       
#[52] gtable_0.3.6       glue_1.8.0         profvis_0.4.0     
#[55] Rcpp_1.1.0         tibble_3.3.0       tidyselect_1.2.1  
...
# Stability Selection with Stable SCAD
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
  ...
  for (lambda_idx in seq_along(candidate_set)) {
    ...
    for (j in 1:B) { 
      ...
      # Fit SCAD-penalized regression
      SCAD_fit <- ncvreg(x_sub, y_sub, penalty = "SCAD", lambda = lambda, penalty.factor = w)
...
tmp_df$Method[tmp_df$Method == "S1"] <- "SCAD" # Rename methods
tmp_df$Method[tmp_df$Method == "S2"] <- "Stable SCAD" # Rename methods
...
metrics_results$Method[metrics_results$Method == 1] <- 'SCAD' # Rename methods
metrics_results$Method[metrics_results$Method == 2] <- 'Stable SCAD' # Rename methods
...
write.csv(results, "SCAD_Full.csv", row.names = FALSE) # Save results to CSV file
