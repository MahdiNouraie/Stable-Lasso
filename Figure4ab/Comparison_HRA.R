# This script generates comparison of initial rankings from the paper
...
# Stability Selection with Stable Lasso (HOLP)
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  ...
  AHOLP <- AirHOLP(x, y, r0 = 1e-6, adapt = FALSE)
  ranked_features <- AHOLP$index_r0  # Ranking of features
...
# Stability Selection with Stable Lasso (Air-HOLP)
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
...
# Stability Selection with Stable Lasso (Ridge-HOLP)
time_taken <- system.time({results <- mclapply(1:100, function(i) {
  set.seed(26 + i) # Set seed for reproducibility
  ...
  AHOLP <- AirHOLP(x, y, r0 = 10, adapt = FALSE)
  ranked_features <- AHOLP$index_r0  # Ranking of features
  w <- 1 - (1/ranked_features)
...
tmp_df$Method[tmp_df$Method == "S1"] <- "HOLP" # Rename methods
tmp_df$Method[tmp_df$Method == "S2"] <- "Air-HOLP" # Rename methods
tmp_df$Method[tmp_df$Method == "S3"] <- "Ridge-HOLP" # Rename methods
...
metrics_results$Method[metrics_results$Method == 1] <- 'HOLP' # Rename methods
metrics_results$Method[metrics_results$Method == 2] <- 'Air-HOLP' # Rename methods
metrics_results$Method[metrics_results$Method == 3] <- 'Ridge-HOLP' # Rename methods
...
write.csv(results, "ComparisonHRA_Full.csv", row.names = FALSE) # Save results to CSV file
